...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ##
##																				   ##
##  ----  ----  ----    MAJ VIA SSH DU BOT TELEGRAM ALARMEBOT    ----  ----  ----  ##
##																				   ##
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ##

## programme pour mettre à jour via SSH le bot. Relance le conteneur si le programme est
## en retard par rapport au dossier distant https://github.com/spystrach/alarmeBot
...
if __name__ == "__main__":
    ...
    with ssh_client:
        ...
        if "retard" in output or "behind" in output or "git pull" in output:
            ...
            if KEEP_DB:
                _, out, err = ssh_client.exec_command(f"docker cp alarme_bot_1:/{basepath_server}/{DB_FILENAME} /tmp/temp_alarme.db")
                out, err = stdout.read(), stderr.read()
                verify_no_errs(out, err)
            # met à jour
            ...
            _, stdout, stderr = ssh_client.exec_command(f"sh {os.path.join(basepath_server, 'restartAlarmeBot.sh')}")
            out, err = stdout.read(), stderr.read()
            verify_no_errs(out, err)
            # recopie la base de donnée dans le conteneur
            if KEEP_DB:
                _, out, err = ssh_client.exec_command(f"docker cp /tmp/temp_alarme.db alarme_bot_1:/{basepath_server}/{DB_FILENAME}")
                out, err = stdout.read(), stderr.read()
                verify_no_errs(out, err)
                # nettoyage
                ssh_client .exec_command("rm /tmp/temp_alarme.db")
        else:
